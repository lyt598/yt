...
DOM.listToArray = function listToArray(likeArray) {
    try {
        return [].slice.call(likeArray);//1、先执行这一行，因为用了try和catch，当这一行报错的时候，会进入catch方法。 这一行在标准浏览器中是可以执行的。 进catch方法说明不是标准浏览器，不支持 [].slice.call方法
    } catch (e) {//2、如果不是标准浏览器才走catch中的代码。 这里先定义一个数组，循环遍历likeArray(即类数组)，把likeArray中的每一项都放到数组中，最后再把数组返回
        var ary = [];
